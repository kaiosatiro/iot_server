...
class TestLogHandlerB:
    ...
    def test_emit_calls_publish_with_formatted_record(self):
        self.channel.status.return_value = True
        self.handler.format = MagicMock(return_value='formatted')
        self.handler.channel.publish = MagicMock()
        ...
        self.handler.channel.publish.assert_called_once_with(message='formatted')
